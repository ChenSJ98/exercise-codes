...
class carp_solver:
    ...
    def solve(self):
        ...
        randPopulation = []
        records = []
        for pSize in range(100,501,100):
            population = []
            print(pSize)
            for i in range(pSize):
                ...
                #random.shuffle(sol1)
                #path, score = self.spliter.split(sol1)
                #randPopulation.append([sol1, path, score])
            population.sort(key=lambda x: x[2])
            records.append(population[0][2])
        #randPopulation.sort(key=lambda x: x[2])
        #population = population[:pSize]
        #randPopulation = randPopulation[:pSize]
        #records = []
        #mrecords = []
        #randRecords = []
        #randMRecords = []
        ...
        for i in range(N):
            ...
            randMinVal = randPopulation[0][2]
            randMaxVal = randPopulation[len(randPopulation) - 1][2]
            print(time.time() - time0, "  gen ", i, ": ", minVal, "----------", maxVal)
            print(time.time() - time0, "  gen rand", i, ": ", randMinVal, "----------", randMaxVal)
            ...
            randOffspring = self.genOffspring(randPopulation)
            randPopulation += randOffspring
            randPopulation.sort(key=lambda x: x[2])
            randPopulation1 = randPopulation[:int(pSize)]
            randPopulation = randPopulation1

            randMRecords.append(randMaxVal)
            randRecords.append(randMinVal)
...
'''
        l = len(records)
        xs = range(100, 100*l + 1, 100)
        p1 = pt.plot(xs, records)
        #p2 = pt.plot(xs, mrecords, label="path scanning -max")
        #p3 = pt.plot(xs, randRecords, label="random - min")
        #p4 = pt.plot(xs, randMRecords, label="random - max")
        #pt.title( "  dataset:" + self.file.split("/")[-1])
        pt.grid()
        #pt.legend(loc = "upper right")
        pt.xlabel("population size")
        pt.ylabel("initial cost")
        pt.show()
        pt.ylabel("inidfsdstial cost")
        pt.show()
    ...
    def genSolution(self, path, taskList):
        ...
        for x in path:
            ...
            for i in range(int((x[0] + 1) / 2), int(x[1] / 2) + 1):
                solution = solution + '(' + str(self.tasks[taskList[i - 1]][0]) + ',' + str(
                    self.tasks[taskList[i - 1]][1]) + '),'
            solution += '0,'
